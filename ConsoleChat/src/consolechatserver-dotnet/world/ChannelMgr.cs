...
namespace CompatibilityStandards {
	...
	public partial class GameFramework {
		...
		public class CChannelMgr : CSingleton< CChannelMgr > {
				...
				if(0 < usId_) {
					...
					if(tKey.channel < GetMaxChannel()) {
						if(tKey.index < iMAX_ROOMS) {
							if(isptr(m_kRoomList[tKey.channel, tKey.index])) {
			...
			public bool
			InUser(INT o, CPlayer kPlayer_) {
				if(o == kPlayer_.GetChannelIndex()) {
					return true;
				} else {
					if(o < GetMaxChannel()) {
						if(GetChannelInfo(o).GetUser() < GetChannelInfo(o).GetMaxUser()) {
							if(0 <= kPlayer_.GetChannelIndex()) {
								g_kServerMgr.OutChannel(kPlayer_.GetChannelIndex(), kPlayer_);
							}
							...
							INT iSize = Marshal.SizeOf(typeof(SInfoChannelInGsToCl));
							kCommand.SetData(tData, iSize);

							BroadcastChannel((UINT)o, kCommand, iSize, kPlayer_);
			...
			public void
			OutUser(INT o, CPlayer kPlayer_) {
				if(o < GetMaxChannel()) {
					if(0 < GetChannelInfo(o).GetUser()) {
						...
						if((0 <= iIndex) && (iIndex < (INT)GetChannelInfo(o).GetMaxUser())) {
							...
							m_tServerInfo.channel_info[o].DecreasedUser();
							...
							INT iSize = Marshal.SizeOf(typeof(SInfoChannelOutGsToCl));
							kCommand.SetData(tData, iSize);
					...
					if(isptr(kRoomHandler)) {
						...
						if(isptr(kRoom)) {
							...
							for(UINT i = 0; i < kRoom.GetTopCount(); ++i) {
								kPlayer = (CPlayerEx)kRoom.GetMember(i);
								if(isptr(kPlayer)) {
									...
									if(isptr(kConnector)) {
										if(iCount >= iMAX_PACKET_ROOM_MEMBER_LIST) {
											...
											INT iSize = (iCount * Marshal.SizeOf(typeof(SRoomMember)));
											kCommand.SetData(tSData, iSize);
					...
					for(INT i = 0; i < m_iUserTopIndex[kPlayer_.GetChannelIndex()]; ++i) {
						if(isptr(m_kUserList[kPlayer_.GetChannelIndex(), i])) {
							if(iCount >= iMAX_PACKET_INFO_USER_LIST) {
								...
								INT iSize = Marshal.SizeOf(typeof(SInfoUserListGsToCl));
								kCommand.SetData(tSData, iSize);
			...
			public SChannelInfo		GetChannelInfo(INT o)		{ return m_tServerInfo.GetChannelInfo(o); }
			public SServerInfo		GetServerInfo()				{ return m_tServerInfo; }

			public void				SetMaxChannel(INT o)		{ m_tServerInfo.SetMaxChannel(o); }
			public INT				GetMaxChannel()				{ return m_tServerInfo.GetMaxChannel(); }
			...
			public void				SetServerInfo(SServerInfo o)	{ m_tServerInfo = new SServerInfo(o); }
			...
			public void 			SetId(INT o, INT p)			{ m_tServerInfo.channel_info[o].SetId((UINT)p); }
			public void				SetMaxUser(INT o, INT p)	{ m_tServerInfo.channel_info[o].SetMaxUser(p); }
			public void				SetUser(INT o, INT p)		{ m_tServerInfo.channel_info[o].SetUser(p); }
			public void				IncreasedUser(INT o)		{ m_tServerInfo.channel_info[o].IncreasedUser(); }
			public void				DecreasedUser(INT o)		{ m_tServerInfo.channel_info[o].DecreasedUser(); }
