...
namespace CompatibilityStandards {
	...
	public partial class GameFramework {
		...
		public class CNetMgr : CSingleton< CNetMgr > {
			...
			Update() {
				...
				if(false == GetMasterNetwork().GetCommandQueue().Empty()) {
					...
					while(isptr(kExtendCommand = (CExtendCommand)(GetMasterNetwork().GetCommandQueue().Take()))) {
						...
						CAllocator< CExtendCommand >.Delete(kExtendCommand);
						++iCount;
						if(200 < iCount) {
							break;
						}
			...
			private CMasterNetwork	m_kMasterNetwork = new CMasterNetwork();
			private CSlaveNetwork	m_kSlaveNetwork = new CSlaveNetwork();
